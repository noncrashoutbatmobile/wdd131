...
console.log("Simple sorted list:", simplesort);
const anotherSort = simpleList.sort(compareFn);
console.log("Another sorted list:", anotherSort);

  function searchList(list, q) {
    function searchCallback(item) {
        // guard missing fields and normalize once
        var qq = String(q).toLowerCase();
        var name = item.name ? String(item.name).toLowerCase() : '';
        var desc = item.description ? String(item.description).toLowerCase() : '';
        var tagsMatch = Array.isArray(item.tags) && item.tags.some(function(tag) {
          return String(tag).toLowerCase().includes(qq);
        });

        return name.includes(qq) || desc.includes(qq) || tagsMatch;
    }
    const filtered = list.filter(searchCallback);

      // numeric sort by distance (extract number from strings like "3 miles")
    ...
    console.log(searchList(hikes, "yellowstone").map(function(h){return h.name;}));
    console.log(searchList(hikes, "moderate").map(function(h){return h.name;}));
    console.log(searchList(hikes, "al").map(function(h){return h.name;}));